...
async function fetchLatestWebhookContent(tokenId) {
    const response = await axios_1.default.get(`/token/${tokenId}/request/latest/raw`, { 'axios-retry': { retries: 4 } });
    return response.data;
}
exports.fetchLatestWebhookContent = fetchLatestWebhookContent;
async function fetchWebhooksContent(tokenId) {
    const response = await axios_1.default.get(`/token/${tokenId}/requests?query=method:POST`, { 'axios-retry': { retries: 4 } });
